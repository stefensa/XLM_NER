...
def train(model, train_iter, dev_iter, params):
    model.train()
    optimizer = optim.Adam(model.parameters(), lr=0.003, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-4)
    ...
    for epoch in trange(params.n_epochs):
        for sentence, tags in train_iter:
            ...
            loss = loss_fn(logits.view(-1, params.num_class), tags.view(-1))
            ...
            if iteration % 20 == 0:
                ...
                if eval_f1 > best_f1:
                    best_f1 = eval_f1
                    save(model, "./dumped_batchsize", iteration)
