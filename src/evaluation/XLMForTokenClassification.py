...
class XLMForTokenClassification:
    ...
    def run(self):
        ...
        self.proj = nn.Sequential(*[
            ...
            nn.Linear(self.embedder.out_dim, params.num_class)
        ]).cuda()
        ...
        for epoch in range(params.n_epochs):
            ...
            # evaluation
            logger.info("XLM - Evaluating epoch %i ..." % epoch)
            with torch.no_grad():
                scores = self.eval()
    ...
    def train(self):
        ...
        while True:
            ...
            # loss
            output = self.proj(self.embedder.get_token_embeddings(x, lengths, positions, langs))         #
            #size(batch_size * seq_len * num_tag)
            loss = F.cross_entropy(output.view(-1, params.num_class), y.view(-1))
