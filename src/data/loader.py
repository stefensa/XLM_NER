...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            mono_data = load_binarized(params.mono_dataset[lang][splt], params) #params.mono_dataset[lang][splt] 指向数据路径
...
def check_data_params(params):
    ...
    # check languages
    params.langs = params.lgs.split('-') if params.lgs != 'debug' else ['en']     # list(lan1,lan2...)
    ...
    params.mlm_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in mlm_steps]    #[('en', None), ('fr', None)]
